...
| `getCellData`        | --                | Sync function for extracting the data from the row. By default, it assumes you passed an array of data as your columns.  | `<T = any>(key: T) => any`                                     | `(d) => d`        |
...
| `renderExpansion`    | --                | Allows rendering a node after the row.                                                                                   | `(key: any) => VNode \| null`                                  | `() => null`      |
